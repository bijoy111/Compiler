...
class ScopeTable
{
    ...
    int var_cnt=0;//used for icg offline to calculate the number of variable used in a scope
    ...
    void print(fstream& logout)
    {
        //fprintf(logout,"\tScopeTable# %d\n",num_of_scopetable);
        logout<<"\tScopeTable# "<<num_of_scopetable<<endl;
        for(int i=0;i<scope_table_size;i++)
        {
    ...
    //used for the icg offline

    void set_VarCount(int cnt)
    {
        var_cnt+=cnt;
    }
    int get_VarCount()
